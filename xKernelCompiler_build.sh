...
export KBUILD_BUILD_HOST=NoName-cloud.Drone.io # Change with your own hostname.
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot${token}/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Compile took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>Sony Xperia XZ1 (poplar)</b> | <b>${KBUILD_COMPILER_STRING}</b>"
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 ignominiOus-poplaR-Beta-${DATE}.zip *
    cd ..
}
check
compile
